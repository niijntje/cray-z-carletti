...
public class Behandling {
	...
	public Delbehandling addDelbehandling(Delbehandling nyDelbehandling, int index) throws IndexOutOfBoundsException{
		...
		//Husk at fŒ generaliseret denne!
		
//		nyDelbehandling = new Toerring("Somethingsomething", this, 10000000, 15000000, 20000000);
		...
		if (index == -1){
			...
			if (index > 0){
				delbehandlinger.get(index-1).setNextDelbehandling(nyDelbehandling);
			}
	...
	public void removeDelbehandling(Delbehandling delbehandling){
		...
		if (indexOf > 0) {
			if(indexOf < delbehandlinger.size()-1){
				delbehandlinger.get(indexOf-1).setNextDelbehandling(delbehandlinger.get(indexOf+1));
			}
			else {
				delbehandlinger.get(indexOf-1).setNextDelbehandling(null);
			}
		}
		delbehandling.setNextDelbehandling(null);	//Er det n¿dvendigt at fjerne denne reference, hvis jeg ikke kommer til at bruge delbehandlingen igen?
	...
	/**
	 * @param index Krav: index >= 0 og index < delbehandlinger.size()
	 * @return
	 */
