...
public class ObjectCreater {
	...
	public void createSomeObjects() {
		...
		MellemlagerPlads pl1 = Service.getInstance().opretMellemlagerPlads("100100101");
		...
		Service.getInstance().opretToerring("T퓊ring1", b, 12*60*60*1000, 15*60*60*1000, 20*60*60*1000, -1);
		...
		Service.getInstance().opretDragering("Dragering 2e", b3, varighed10, -1);
		...
		Produkttype p = Service.getInstance().opretProdukttype("Lakridspinde 3A", "Hvide lakridspinde med kernest퓊relse 2.\nOpskrift: B2", b);
		Produkttype p2 = Service.getInstance().opretProdukttype("Skumbananer", "Dejlig skum overtrukket af chokolade", b);
		Produkttype pt1 = Service.getInstance().opretProdukttype("Lakridspinde 1", "Gr퓆ne lakridspinde med kernest퓊relse 3.\nOpskrift: B3", b3);
		Produkttype pt2 = Service.getInstance().opretProdukttype("M똭egrus 2B", "Rester fra lakridspinde med varierende kernest퓊relse.\nOpskrift: B3", b3);
		...
		for (Mellemvare m: pa1.getMellemvarer()){
			if (m.getProdukttype() == pt1){
				m.setIgangvaerendeDelbehandling(m.getIgangvaerendeDelbehandling().getNextDelbehandling());
	 ...
	 * @param dato
	 */
	private void opretMellemvare(String bakkestregkode, Produkttype produkttype, Palle palle,
			GregorianCalendar dato) {
