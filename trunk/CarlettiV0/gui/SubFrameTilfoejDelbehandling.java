...
/**
 * Til administration af behandlinger - tilføjelse af delbehandling
 * 
		...
		btnTilfoej.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (cboxmodel.getSelectedItem() == "Dragéring") {
					...
					long varighed = Validering
							.varighedStringTilMillisekunder(txtMin.getText());
					Service.getInstance().opretDragering(txtNavn.getText(),
							getBehandling(), varighed, index);
				}
				if (cboxmodel.getSelectedItem() == "Toerring") {
					long minTid = Validering
							.varighedStringTilMillisekunder(txtMin.getText());
					long idealTid = Validering
							.varighedStringTilMillisekunder(txtIdeal.getText());
					long maxTid = Validering
							.varighedStringTilMillisekunder(txtmaxTid.getText());
					int index = -1;
					try {
						...
						if (txtIndex.getText().equals("")) {
							...
							System.out.println("Advarsel: "
									+ txtIndex.getText()
									+ " er ikke et gyldigt heltal.\nIndex "
									+ index + " benyttes");
						}
					}
					Service.getInstance().opretToerring(txtNavn.getText(),
							getBehandling(), minTid, idealTid, maxTid, index);
				}
				notifyObservers();
										...
										.addGroup(
												gl_contentPane
														...
														.addComponent(
																...
																GroupLayout.DEFAULT_SIZE,
																GroupLayout.PREFERRED_SIZE))
										.addGap(30).addComponent(btnTilfoej)
